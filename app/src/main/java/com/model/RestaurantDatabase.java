...
public class RestaurantDatabase {
    ...
    public boolean AddtoDataBase(Restaurant place)
    {

        final Map<String,Object> restaurant=new HashMap<>();
        restaurant.put("name",place.getName());
        restaurant.put("address",place.getVicinity());
        restaurant.put("rating",0);
        restaurant.put("small_address","");
        restaurant.put("been_here",0);
        restaurant.put("reviews",0);
        restaurant.put("bookmark",0);
        restaurant.put("sum",0);
        restaurant.put("average_cost",500);
        restaurant.put("phonenumber","");
        restaurant.put("placeid",place.getId());
    ...
    public boolean addReview(String Name, String placeId, int rating, final String review)
    {
        ...
        db.collection("Restaurant").document(pl).collection("Reviews").document(personEmail).set(Review).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if(task.isSuccessful()){
                    Log.e("RestaurantDatabase", "review added");
                }else {
                    Log.e("RestaurantDatabase", "review not added");
                }
            }
        });
